...
export class ReservationController {
    ...
    async GetHistoryAsync(
        ...
        const response = new ApiResponse<List<ReservationVO>>();
        try {
            const result = await this._reservationService.GetHistoryAsync(userId);

            if (result.isFailed) {
                response.success = false;
                response.message = result.errors ? result.errors[0] : ConstantsMessagesReservation.ErrorGetAll;
                return StatusCode(res, StatusCodes.STATUS_400_BAD_REQUEST, response);
            }
